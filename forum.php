...
switch ($aktion) {
	...
	case "kategorie_neu":
		// Forum anzeigen
		$box = $t['forum_kopfzeile_kategorie_anlegen'];
	...
	case "kategorie_anlegen":
		...
		if (!$fehlermeldung) {
			...
			$box = $t['forum_kopfzeile_forenuebersicht'];
			$text .= forum_liste();
			zeige_tabelle_zentriert($box, $text);
		} else {
	...
	case "kategorie_editieren":
		...
		if (!$fehlermeldung) {
			...
			// Forenübersicht anzeigen
			$box = $t['forum_kopfzeile_kategorie_editieren'];
			$text .= forum_liste();
			zeige_tabelle_zentriert($box, $text);
		} else {
			$text .= hinweis($fehlermeldung, "fehler");
	...
	case "forum_neu":
		// Forum erstellen
		$box = $t['forum_kopfzeile_forum_anlegen'];
		$text .= maske_forum();
		zeige_tabelle_zentriert($box, $text);
		break;
	...
	case "forum_edit":
		// Forum editieren
		$box = $t['forum_kopfzeile_forum_editieren'];
	...
	case "posting_anlegen":
		...
		if ($schreibrechte && !$thread_gesperrt) {
			...
			if (!$fehlermeldung) {
				...
				// Neues Thema erstellen
				$box = $t['forum_kopfzeile_thema_erstellen'];
	...
	case "edit":
		// Neues Thema erstellen
		$box = $t['forum_kopfzeile_thema_editieren'];
	...
	case "verschiebe_posting":
		if ($forum_admin) {
			// Thema verschieben
			$box = $t['forum_kopfzeile_thema_verschieben'];
