...
class chatClass {
	public static function getRestChatLines($last_time_id) {
		...
		foreach($result as $zaehler => $row) {
			...
			if ($ausgeben) {
				$query_usr = pdoQuery("SELECT `u_id`,`u_farbe`, `u_level` FROM `user` WHERE `u_id` = :u_id", [':u_id'=>$u_id]);
				...
				if( $row['c_typ'] == 'S' ) {
					...
					if ($benutzerdaten['u_layout_chat_darstellung'] == '0') {
						...
						if ( str_contains($row['c_text'], $gesuchter_nutzer) ) {
							$vonuserid = $ava . " <span class=\"nachrichten_privat\" title=\"". $times ."\"><b>" . $row['c_von_user'] . "</b>: ";
						} else {
							$vonuserid = $ava . " <span style=\"". $usr_farbe ."\" title=\"". $times ."\"><b>" . $row['c_von_user'] . "</b>: ";
						}
					} else {
						$vonuserid = $ava . " <span style=\"". $usr_farbe ."\" title=\"". $times ."\"><b>" . $row['c_von_user'] . "</b>: ";
					}
				...
				if( $row['c_typ'] == 'P' ) {
					...
					if ($benutzerdaten['u_layout_chat_darstellung'] == '0') {
						...
						$vonuserid = $ava . " <span style=\"". $usr_farbe ."\" title=\"". $times ."\"><b>". $row['c_von_user'] ."&nbsp;(<a href=\"#\" onMouseOver=\"return(true)\" onClick=\"appendtext_chat('/msg ". $row['c_von_user'] ." '); return(false)\">privat</a>):</b> ";
					}
				}
