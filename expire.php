...
if ($expire_privat) {
	...
	if ($rows > 0) {
		...
		while ($i < $rows) {
			...
			// Chat-Zeile löschen
			$query = "DELETE FROM chat WHERE c_id=$row->c_id";
			$result3 = sqlUpdate($query);
			$i++;
		}
		mysqli_free_result($result);
	...
	$query = "DELETE FROM chat WHERE (UNIX_TIMESTAMP(NOW())-UNIX_TIMESTAMP(c_zeit)) > 900 AND c_typ='P' OR c_zeit='' OR c_zeit=0";
...
$query = "SELECT SQL_BUFFER_RESULT r_id,r_name FROM raum LEFT JOIN online ON o_user=r_besitzer WHERE r_status2 LIKE 'T' AND o_timestamp IS NULL";
$result = sqlQuery($query);

if ($result && mysqli_num_rows($result) > 0) {
	while ($row = mysqli_fetch_object($result)) {
		// Ist der Raum leer?
		$query = "SELECT o_id from raum,online WHERE r_id=$row->r_id AND o_raum=r_id";
		$result3 = sqlQuery($query);
		if ($result3) {
...
if ($zeit == "03:10") {
	...
	if ($result) {
		while ($row = mysqli_fetch_object($result)) {
			$query = "DELETE FROM bild WHERE b_id=" . $row->b_id;
			$result3 = sqlUpdate($query);
		}
	}
	mysqli_free_result($result);
	...
	if ($result) {
		while ($row = mysqli_fetch_object($result)) {
			$query = "DELETE FROM mail WHERE m_id=" . $row->m_id;
			$result3 = sqlUpdate($query);
		}
	}
	mysqli_free_result($result);
	...
	if ($result) {
		while ($row = mysqli_fetch_object($result)) {
			$query = "DELETE FROM aktion WHERE a_id=" . $row->a_id;
			$result3 = sqlUpdate($query);
		}
	}
	mysqli_free_result($result);
	...
	if ($result) {
		while ($row = mysqli_fetch_object($result)) {
			$query = "DELETE FROM userinfo WHERE ui_id=" . $row->ui_id;
			$result3 = sqlUpdate($query);
		}
	}
	mysqli_free_result($result);
...
if ( isset($anzahl_online) && ($anzahl_online > 0) ) {
	$query = "SELECT SQL_BUFFER_RESULT c_users FROM statistiken WHERE DATE_FORMAT(c_timestamp,'%Y-%m-%d %H') = '$currentdate' ORDER BY c_users DESC";
	$r0 = sqlQuery($query);
	...
	if ($r0) {
		...
		if ($n == 0) {
			...
			$query = "INSERT INTO statistiken (c_timestamp, c_users) VALUES (NOW(),$anzahl_online)";
			sqlUpdate($query);
		} else {
			...
			if ($anzahl_online > $currentnr) {
				$query = "UPDATE statistiken SET c_users=$anzahl_online WHERE DATE_FORMAT(c_timestamp,'%Y-%m-%d %H') = '$currentdate'";
				sqlUpdate($query);
			}
		}
	}
